...
export const PopularMuseums: React.FC = () => {
  ...
  const { data: museums, isLoading } = trpc.places.getBySlugs.useQuery(
    { slugs: MUSEUM_SLUGS },
    { staleTime: 60000 },
  );
  ...
  const handleCardClick = (p: {
    ...
    const url = `/search?country=${encodeURIComponent(p.country.toLowerCase())}&state=${encodeURIComponent(p.state.toLowerCase())}&city=${encodeURIComponent(p.city.toLowerCase())}&lookFor=${p.slug}&destinationType=museum`;
    router.push(url as Route);
  };

  const handleBrowseAll = () => {
    router.push('/places?type=museum' as Route);
  };
            ...
            Array.from({ length: SKELETON_COUNT }).map((_, i) => (
              ...
              >
                ...
                <div className='p-2 leading-none'>
                  <p className='text-xl text-background'>Museum</p>
